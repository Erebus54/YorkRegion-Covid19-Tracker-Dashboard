...
```{r}
valueBox(paste(round(sum(TargetPHU$Case_AcquisitionInfo == "Missing Information")/nrow(TargetPHU)*100, 1), "%"), 
         icon = "ion-help-circled")
```

### No known epi link
```{r}
valueBox(paste(round(sum(TargetPHU$Case_AcquisitionInfo == "No known epi link")/nrow(TargetPHU)*100, 1), "%"), 
         icon = "ion-help-circled")
```

### Unspecified epi link
```{r}
valueBox(paste(round(sum(TargetPHU$Case_AcquisitionInfo == "Unspecified epi link")/nrow(TargetPHU)*100, 1), "%"), 
         icon = "ion-help-circled")
```
...
fig <- plot_ly(x = ~acq_daily$Accurate_Episode_Date, y = ~acq_daily$CC, type = 'bar', name = 'Close Contact', 
               marker = list(color = '#E01E5A'))

fig <- fig %>% add_trace(x = ~acq_daily$Accurate_Episode_Date, y = ~acq_daily$OB, name = 'Outbreak Related', 
                         marker = list(color = '#F4B400'))

fig <- fig %>% add_trace(x = ~acq_daily$Accurate_Episode_Date, y = ~acq_daily$Travel, name = 'Travel', 
                         marker = list(color ='#25D366'))
...
cacot <- merge(x = cacot, y = dailyPHU[ , c("Accurate_Episode_Date", "Cases")], by = "Accurate_Episode_Date")
cacot <- cacot %>% 
...
plot_ly(data = cacot, x = ~Accurate_Episode_Date, y = ~CC_Pct, 
...
```{r}
plot_ly(acq_daily, x = ~Accurate_Episode_Date, y = ~OB_csum, type = 'scatter', mode = 'none', fill = 'tozeroy', 
        name = 'Outbreak', 
        fillcolor = 'rgba(244, 180, 0, 0.7)',
        opacity = 0.8) %>% 
  
  
  add_trace(x = ~Accurate_Episode_Date,y = ~CC_csum, name = "Close Contact", fill = 'tozeroy',
            fillcolor = 'rgba(224, 30, 90, 0.7)',
            opacity = 0.8) %>% 
  
  add_trace(x = ~Accurate_Episode_Date,y = ~Travel_csum, name = "Travel", fill = 'tozeroy',
