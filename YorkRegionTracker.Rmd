...
plot_ly(x = ~Accurate_Episode_Date, 
  ...
  add_trace(y = ~new_deaths, 
            ...
            text = ~paste("Change in Deaths from Yesterday: ", new_resolved ,sep = ""),
            hoverinfo = "text") %>% 
  layout(title = "", 
         xaxis = list(title = '', rangeselector = list(buttons = buttons_time)),
...
plot_ly(acq_daily, x = ~Case_Reported_Date, y = ~CC_csum, 
        type = 'scatter', mode = 'none', fill = 'tozeroy', 
        name = 'Close Contact', 
        fillcolor = 'rgba(224, 30, 90, 0.7)',
        opacity = 0.8) %>% 
  
  add_trace(y = ~Unknown_csum, name = "Uknown", 
            fill = 'tozeroy',
            fillcolor = 'rgba(220, 220, 220, 0.7)',
            opacity = 0.8) %>% 
  
  add_trace(y = ~OB_csum, name = "Outbreak", 
            fill = 'tozeroy',
            fillcolor = 'rgba(244, 180, 0, 0.7)',
            opacity = 0.8) %>% 
  
  add_trace(y = ~Travel_csum, name = "Travel", 
            fill = 'tozeroy',
...
plot_ly(genders, labels = ~Client_Gender, values = ~n, type = 'pie',
               textposition = 'inside',
               textinfo = 'label+percent',
               insidetextfont = list(color = '#FFFFFF'),
               hoverinfo = 'text',
